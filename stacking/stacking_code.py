...
gbdt_param = {'bagging_fraction': 0.7744376536407631, 'bagging_freq': 0, 'boosting': 0,
              ...
              'num_leaves': 135, 'num_rounds': 2000, 'num_threads': 64, 'objective': 'binary'}

#0.20563000000000003
goss_param = {'num_thread': 64, 'num_leaves': 80, 'metric': 'binary_error', 'objective': 'binary', 'num_round': 2000,
              'learning_rate': 0.01, 'feature_fraction': 0.8, 'boosting': 'goss'}

#0.20643000000000003
dart_param = {'num_thread': 64, 'num_leaves': 128, 'metric': 'binary_error', 'objective': 'binary', 'num_round': 2000,
              'learning_rate': 0.07, 'feature_fraction': 0.4, 'bagging_fraction': 0.9, 'bagging_freq': 10, 'boosting': 'dart'}

bst_gbdt = lgb.train(gbdt_param, train_data)
bst_goss = lgb.train(goss_param, train_data)
bst_dart = lgb.train(dart_param, train_data)
...
lgb_out['gbdt'] = gbdt_out
lgb_out['goss'] = goss_out
lgb_out['dart'] = dart_out

lgb_out.to_csv('lgb_out.csv')